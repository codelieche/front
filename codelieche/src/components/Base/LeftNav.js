...
export const NavItem = ({item, index, collapsed, defaultOpenKey}) => {
    ...
    if(item.children && Array.isArray(item.children)){
        childrenElements = item.children.map((item, index) => {
        ...
        if(Array.isArray(childrenElements) &&  childrenElements.length > 0){
            ...
            if(item.is_link && item.link){
                ...
                if(item.title && item.level > 1){
                    ...
                    menuTitleElement = (
                        // 如果是一级层单，不要显示Title，但是需要显示提示【标题】
                        <Popover 
                            title={item.title}
                            content={<div className="collapsed-nav-children"></div>} 
                            placement="rightTop">
                                <div className="item">
                                    <div className="title">
                                        <Icon type={item.icon ? item.icon : "angle-right"}></Icon>
    ...
    if(Array.isArray(childrenElements) && childrenElements.length > 0){
        titleElement = (
            <div>
                ...
                <div 
                    className="title"
                    style={{paddingLeft: levelPaddingLeft * item.level}}
        ...
        if(item.is_link && item.link){
            titleElement = (
                <a href={item.link} target={item.target} rel="noopener noreferrer">
                    <div className="title" style={{paddingLeft: levelPaddingLeft * item.level}}>
                        {iconElement}
                        {item.title ? item.title : "无标题"} 
                        {rightIconElement}
                    </div>
...
export const LeftSiderNav = (props) => {
    ...
    // 是否显示左侧导航
    const { showLeftSider } = useContext(GlobalContext);

    // 获取用用户导航数据
    const fetchNavData = useCallback(() => {
        const url = "/api/v1/config/menu/user";
        fetchApi.Get(url)
        .then(data => {
